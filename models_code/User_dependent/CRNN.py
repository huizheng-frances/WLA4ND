...
def confu_matrix_plot(true_multiclass,pred_multiclass,i):
    labels = ['read', 'wr', 'write','type', 'rest', 'off']
    ...
    plt.savefig('results/confusion_matrix_CRNN_'+str(i)+'.png', dpi=150)
...
def main():
    ...
    for i in range(len(file_list)):
        ...
        #print(confu_matrix)
        numpy.savetxt("confu_matrix/confusion_matrix_CRNN_" + str(i) + ".csv", X=confu_matrix.astype(int),
                      delimiter=', ',
                      fmt='%.0f')
        ...
        print(confu_matrix)
        numpy.savetxt("confu_matrix/confusion_matrix_CRNN-" + str(i) + "-nor.csv", X=confu_matrix.astype(float),
                      delimiter=', ',
                      fmt='%.00f')
