...
def confu_matrix_plot(true_multiclass,pred_multiclass,i):
    ...
    plt.savefig('results/confusion_matrix_M_LSTM_'+str(i)+'.png', dpi=150)
...
def main():
    ...
    file = open('results/M_LSTM.txt', 'w')
    ...
    for i in range(len(file_list)):
        ...
        print(confu_matrix)
        numpy.savetxt("confu_matrix/confusion_matrix_M-LSTM_" + str(i) + ".csv", X=confu_matrix.astype(int),
                      delimiter=', ',
                      fmt='%.0f')
        confu_matrix = confu_matrix / np.sum(confu_matrix) * 1000
        print(confu_matrix)
        numpy.savetxt("confu_matrix/confusion_matrix_M-LSTM-" + str(i) + "-nor.csv", X=confu_matrix.astype(float),
                      delimiter=', ',
                      fmt='%.00f')
