...
def build_model(trainX,
    ...
    cnn_out = Convolution1D(nb_filter=con_layer1, filter_length=con_layer1_filter,activation='relu')(input)
    cnn_out = MaxPooling1D(3)(cnn_out)
    lstm_out = LSTM(lstm_layer, activation='relu',dropout=drop, recurrent_dropout=r_drop)(cnn_out)
...
def main():
    ...
    for i in [1, 2, 3, 5, 6, 7, 8, 9]:
        ...
        model = build_model(train_X,con_layer1, con_layer1_filter,  lstm_layer, drop, r_drop, shared_layer, dense_num, n_labels)
        ...
        #file = open('results/CRNN'+ps+'.txt', 'w')
        ...
        print(confu_matrix)
        numpy.savetxt("confu_matrix/CRNN/confusion_matrix_CRNN-"+ ps +".csv", X=confu_matrix.astype(int), delimiter=', ',fmt ='%.0f')
        confu_matrix = confu_matrix / np.sum(confu_matrix) * 1000
        print(confu_matrix)
        numpy.savetxt("confu_matrix/CRNN/confusion_CRNN-" + ps + "-nor.csv", X=confu_matrix.astype(float),
                      delimiter=', ',
                      fmt='%.00f')
