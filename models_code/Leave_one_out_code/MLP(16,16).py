...
def build_model(trainX,shared_layer,dense_num, n_labels):
    ...
    dense_out = Dense(dense_num, activation='relu')(input)

    dense_shared = Dense(shared_layer, activation='relu')(dense_out)
...
def main():
    ...
    for i in [1, 2, 3, 5, 6, 7, 8, 9]:
        ...
        model = build_model(train_X, shared_layer,dense_num, n_labels)
        ...
        #file = open('results/MLP' + ps + '.txt', 'w')
        ...
        print(confu_matrix)
        numpy.savetxt("confu_matrix/confusion_matrix_MLP" + ps + ".csv", X=confu_matrix.astype(int), delimiter=', ', fmt='%.0f')
